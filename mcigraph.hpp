...
public:
  ...
  void draw_image(std::string filename, int x = 0, int y = 0, int scale = 1) {
    auto tex = _texcache.load(filename);
    int w, h;
    SDL_QueryTexture(tex, NULL, NULL, &w, &h);
    SDL_Rect dest_rect = {x, y, w*scale, h*scale};
...
const auto KEY_LCTRL = SDL_SCANCODE_LCTRL;
...
inline void draw_image(std::string filename, int x = 0, int y = 0, int scale = 1) {
  mcigraph::MciGraph::get_instance().draw_image(filename, x, y, scale);
}
